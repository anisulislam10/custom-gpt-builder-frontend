...
export const saveFlow = createAsyncThunk(
  ...
  async ({ userId, nodes, edges, flowName, websiteDomain }, { rejectWithValue }) => {
    try {
      const response = await axios.post(`http://165.227.120.144/api/flow/${userId}`, {
        nodes,
        edges,
...
export const loadFlows = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://165.227.120.144/api/flow/${userId}`);
      return response.data;
    } catch (error) {
...
export const loadFlows2 = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://165.227.120.144/api/flow/user/${userId}`);
      return response.data;
    } catch (error) {
      return rejectWithValue(handleApiError(error).message);
    }
...
export const loadFlow = createAsyncThunk(
  ...
  async ({ userId, flowId }, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://165.227.120.144/api/flow/${userId}/${flowId}`);
...
export const updateFlow = createAsyncThunk(
  ...
  async ({ userId, flowId, nodes, edges, websiteDomain }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`http://165.227.120.144/api/flow/${userId}/${flowId}`, {
        nodes,
        edges,
        websiteDomain
      });
...
export const deleteFlow = createAsyncThunk(
  ...
  async ({ userId, flowId }, { rejectWithValue }) => {
    try {
      await axios.delete(`http://165.227.120.144/api/flow/${userId}/${flowId}`);
      return flowId;
    } catch (error) {
      return rejectWithValue(handleApiError(error).message);
    }
