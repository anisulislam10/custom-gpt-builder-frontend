// app/statistics/[flowId]/page.tsx
'use client';
...
export default function StatisticsPage() {
  ...
  console.log("jsx",id)
    ...
    const flowId = id; // ✅ your desired ID

  const [stats, setStats] = useState({ daily: 0, weekly: 0, monthly: 0, yearly: 0, allTime: 0 });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
console.log("jsx",flowId)
  // Fetch statistics
  useEffect(() => {
  ...
  const barData = {
    ...
    datasets: [
      {
        label: 'Unique Users',
        data: [stats.daily, stats.weekly, stats.monthly, stats.yearly, stats.allTime],
  ...
  const pieData = {
    ...
    datasets: [
      {
        data: [stats.daily, stats.weekly, stats.monthly, stats.yearly],
        backgroundColor: [
          'rgba(79, 70, 229, 0.8)',
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-12">
          {['daily', 'weekly', 'monthly', 'yearly', 'allTime'].map((period) => (
            ...
            >
              ...
              <p className="text-3xl font-bold text-indigo-600">{stats[period]}</p>
